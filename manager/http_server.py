...
class WebUI:
    ...
    def __init__(self, disable_register=False):
    ...
    def setup_routes(self):
        ...
        register_handler = RegisterHandler(
            self.user_manager, self.disable_register)
        config_handler = ConfigHandler(self.session_manager)

        # Public APIs
        self.app.router.add_post('/api/login', login_handler.handle_login)
        self.app.router.add_post(
            '/api/register', register_handler.handle_register)

        # Protected APIs
        self.app.router.add_get(
            '/api/config/devices', self.auth_wrapper(config_handler.get_private_configs))

        self.app.router.add_get('/api/config/module-options',
                                self.auth_wrapper(config_handler.get_module_options))
        self.app.router.add_post('/api/config/save_device_config',
                                 self.auth_wrapper(config_handler.save_device_config))
        self.app.router.add_post(
            '/api/config/delete_device', self.auth_wrapper(config_handler.delete_device_config))
        self.app.router.add_post(
            '/api/config/bind_device', self.auth_wrapper(config_handler.bind_device))

        # 添加静态文件服务
        self.app.router.add_static(
            '/assets/', path=os.path.join(self.static_path, 'assets'))
        ...
        async def wrapper(request):
            ...
            if not session_id:
                logger.bind(tag=TAG).warning(
                    "No session_id in Authorization header")
                return web.json_response({'error': 'Unauthorized'}, status=401)
